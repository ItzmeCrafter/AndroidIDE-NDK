...
1. Download the NDK installation script:
   ...
   ```bash
   cd && pkg upgrade && pkg install wget && wget https://github.com/ItzmeCrafter/AndroidIDE-NDK/raw/main/ndk-install.sh --no-verbose --show-progress -N && chmod +x ndk-install.sh && bash ndk-install.sh
   ```
